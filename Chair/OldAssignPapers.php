...
//
if (IsSet($assignPapers)) {
  if (!IsSet($reviewer) || $reviewer==-1) {
    $Conf->errorMsg("You need to select a reviewer.");
  } else {
    if (IsSet($Primary)) {
      for($i=0; $i < sizeof($Primary); $i++) {
	$paper=$Primary[$i];
	$result = $Conf->qe("SELECT reviewer FROM PrimaryReviewer WHERE reviewer='$reviewer' AND  paperId='$paper'");
	if (DB::isError($result) ||  $result->numRows() == 0) {

	  $result=$Conf->qe("INSERT INTO PrimaryReviewer SET reviewer='$reviewer', paperId='$paper'");

	  if ( !DB::isError($result) ) {
	    $Conf->infoMsg("Added primary reviewer for paper $paper");
	    $Conf->log("Added primary reviewer $reviewer for paper $paper", $Me);
	  } else {
	...
	$result = $Conf->qe("SELECT reviewer FROM SecondaryReviewer "
			    . " WHERE reviewer='$reviewer' AND  paperId='$paper'");

	if (DB::isError($result) ||  $result->numRows() == 0) {

	  $result=$Conf->qe("INSERT INTO SecondaryReviewer SET "
			     . " reviewer='$reviewer', paperId='$paper'");
...
?>
       ...
       <td>
       <INPUT TYPE=checkbox NAME=Primary[] VALUE='<?echo $paperId?>'> 1st? <br>
       <INPUT TYPE=checkbox NAME=Secondary[] VALUE='<?echo $paperId ?>'> 2nd? <br>
       </td> 
       </tr>
       </table>
       </td>
       ...
       <td valign="center" align="center"> <? echo Num($allReviewRequest[$paperId]) ?> </td>
       <td valign="center" align="center"> <? echo Num($allStartedReviews[$paperId]) ?> </td>
       <td valign="center" align="center"> <? echo Num($allFinishedReviews[$paperId]) ?> </td>
