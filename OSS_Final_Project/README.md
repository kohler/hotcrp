...
* 지금 현재로서는 서버 관리자가 직접 주기적으로 DB에서 수동으로 csv 파일을 업데이트 해줘야하지만, 추후에는 데이터베이스에 존재하는 PaperTopic의 데이터를 바로 가져와서 실시간으로 시각화그래프가 업데이트 되게 하여, 사용자가 원하는 때에 바로 업데이트된 시각화그래프 볼 수 있게 구현할 예정입니다.
* 우리가 현재 연구 및 개발한 논문 관리 사이트의 시각화를 좀 더 보완하여, 논문을 써볼 예정이며, 아직 학회를 정하지는 않았지만 학회를 좀 더 조사하여 논문을 제출해 보려고 시도할 예정입니다. 
* 또한 이 개발 결과를 가지고 여러 중학교나 고등학교, 또한 여러 학회들까지 이 사이트를 잘 활용해 볼 수 있도록 제안을 해볼 예정입니다. 
