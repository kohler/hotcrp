...
use TokenInfo, TokenScope, Signin_Page, Authorization_Token;
...
class OAuthClient {
    ...
    static function make_dynamic($x, $ctok) {
        ...
        foreach ($ctok->data("redirect_uris") ?? [] as $uri) {
            $oac->redirect_uris[] = $uri;
        }
        return $oac;
    }
}
...
class Authorize_Page {
    ...
    private function handle_authconfirm() {
        ...
        if (!$tok->data("email")) {
            $tok->change_data("email", $this->viewer->email)
    ...
    static function oauthtoken_api(Contact $user, Qrequest $qreq) {
        return (new Authorize_Page($user, $qreq))->handle_oauthtoken();
    }
