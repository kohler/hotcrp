...
class PaperOption implements JsonSerializable {
    ...
    static function make_readable_formid($s) {
        ...
        if (!preg_match('/\A(?:title|paper|submission|final|authors?|blind|nonblind|contacts?|abstract|topics?|pc_conflicts?|pcconf|collaborators?|comments?|reviews?|sclass|status.*|submit.*|fold.*|[a-z]?[a-z]?[-_0-9].*|has[-_].*|)\z/', $s)) {
            return $s;
        }
        return "sf-" . $s;
    }
