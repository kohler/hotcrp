...
final class TokenScope {
    ...
    static public $scopes = [
        ...
        "read" => 0x11111111, "write" => 0x33333333, "admin" => -1,
        "paper:read" => 0x11111, "paper:write" => 0x33333, "paper:admin" => 0x77777,
        "submission:read" => 0x11, "submission:write" => 0x33, "submission:admin" => 0x77,
        "submeta:read" => 0x1, "submeta:write" => 0x3, "submeta:admin" => 0x7,
        "document:read" => 0x11, "document:write" => 0x33, "document:admin" => 0x77,
        ...
        "other:read" => 0x100000, "other:write" => 0x300000, "other:admin" => 0x700000
    ];

    /** @var list<int> */
    static public $scope_subset_masks = [
        0xFFFFF /* paper */, 0xFF /* submission */, 0xF /* submeta */,
        0xF00 /* review */, 0xF000 /* tag */, 0xF0000 /* comment */,
        0xF00000 /* other */
    ];
     ...
     * @param ?Contact $user */
    private function __construct($all_bits, $rest, $user) {
    ...
    static function unparse_bits($bits) {
        if ($bits === ~0 || $bits < -1) {
            ...
            return ["none"];
        } else if (($k = array_search($bits, self::$scopes, true)) !== false) {
            return [$k];
        }
        $a = [];
        foreach (self::$scope_subset_masks as $m) {
            if (($bits & $m) !== 0
                && ($k = array_search($bits & $m, self::$scopes, true)) !== false) {
                $a[] = $k;
                $bits &= ~$m;
            }
        }
