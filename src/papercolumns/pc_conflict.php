...
class Conflict_PaperColumn extends PaperColumn {
    ...
    function decoration_spec() {
        return ["simple", "edit", "pin=yes|no|all|none|unconflicted|conflicted"];
    }
    function prepare(PaperList $pl, $visible) {
        $this->contact = $this->contact ? : $pl->reviewer_user();
        $this->not_me = $this->contact->contactId !== $pl->user->contactId;
        $this->editable = $this->decoration_value("edit") ?? $this->editable;
        if ($this->editable) {
            $this->simple = $this->decoration_value("simple") ?? false;
            $pin = $this->decoration_value("pin") ?? "none";
            $this->pin_no = $pin === "all" || $pin === "yes" || $pin === "unconflicted";
            $this->pin_yes = $pin === "all" || $pin === "yes" || $pin === "conflicted";
