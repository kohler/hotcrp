...
class Responses_SettingParser extends SettingParser {
    ...
    function parse_req(SettingValues $sv, Si $si) {
        ...
        foreach ($roundnames_set as $i) {
            $isuf = $i ? "_$i" : "";
            if (($v = $sv->base_parse_req("resp_open$isuf")) !== null) {
                $sv->save("resp_open$isuf", $v <= 0 ? null : $v);
            }
            if (($v = $sv->base_parse_req("resp_done$isuf")) !== null) {
                $sv->save("resp_done$isuf", $v <= 0 ? null : $v);
            }
            if (($v = $sv->base_parse_req("resp_grace$isuf")) !== null) {
                $sv->save("resp_grace$isuf", $v <= 0 ? null : $v);
            }
            if (($v = $sv->base_parse_req("resp_words$isuf")) !== null) {
                $sv->save("resp_words$isuf", $v < 0 ? null : $v);
            }
            if (($v = $sv->base_parse_req("resp_search$isuf")) !== null) {
