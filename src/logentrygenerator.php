<?php
// logentrygenerator.php -- HotCRP action log entry generator
...
class LogEntryGenerator {
     ...
     * @return $this */
    function set_paper_set(PaperInfoSet $rowset) {
        $this->rowset = $rowset;
        return $this;
    }
        ...
        while ($last > $this->first_index + count($this->rows)
            ...
            while (($row = $result->fetch_object("LogEntry"))) {
                ...
                if (($this->filter
                    ...
                    || ($this->action_regex
                        && !preg_match("\x01{$this->action_regex}\x01i", $row->cleaned_action()))) {
                    continue;
                }
                ...
                if ($this->consolidate_row
                    ...
                    $this->consolidate_row->merge($row);
                    continue;
                }
